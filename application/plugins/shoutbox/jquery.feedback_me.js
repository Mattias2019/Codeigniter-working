...
var fm = (function ($) {
			...
			success: function (data) {
				...
				if (fm_options.delayed_close === true) {
					if (fm_options.delayed_options.custom_html_success === undefined) {
						...
						slideBack(fm_options, $fm_trigger, $fm_content);
						$("body").append('<div class="parent_feedback_msg_cls"><div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><span class="glyphicon glyphicon-ok"></span> <strong>Success</strong><hr class="message-inner-separator"><p>Your feedback has been sent </p></div></div>');
					} else {
						$fm_content.find('.feedback_submit').text(fm_options.submit_label);
						slideBack(fm_options, $fm_trigger, $fm_content);
			...
			error: function (ob, errStr) {
				...
				if (fm_options.delayed_close === true) {
					if (fm_options.delayed_options.custom_html_fail === undefined) {
						...
						$fm_content.find('.feedback_submit').text(fm_options.submit_label);
						$("body").append('<div class="parent_feedback_msg_cls"><div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><span class="glyphicon glyphicon-hand-right"></span> <strong>Failed</strong><hr class="message-inner-separator"><p>Your feedback has not been sent .</p></div></div>');
	...
	function init(options) {
		...
		var default_options = {
			...
			radio_button_list_labels : ["Suggestion", "Missing Function", "Error Report", "Question"],
			...
			delayed_close : true,
			delayed_options : {
				delay_success_milliseconds : 30000,
				delay_fail_milliseconds : 30000,
				sending : "Sending...",
				send_fail : "Sending failed.",
				send_success : "Your feedback has been sent",
				fail_color : undefined,
				success_color : undefined,
